...
request message on the stream.  However, in order to do this, we need to
defer sending initial metadata until the client sends the first message,
which is a change from the normal behavior.  This new behavior will only
take effect if there is a header extraction config specified for the
call.
...
[
  {
    ...
    // If absent, the entire payload field will be used.
    ...
    // Ignored if the 'delimiterCharacter' field is not present.
