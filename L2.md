...
##### 1.  Create `PollingType` enums to mirror the gRPC C Core's enums
C++ provides a `CompletionQueue` type class.

`CompletionQueue` has two methods `Next()` and `Pluck()` but only `Next()` is
public.
...
- Make the public constructor to call the private constructor with
  `GRPC_CQ_NEXT` and `GRPC_CQ_DEFAULT_POLLING`

- Change any internal code that uses `CompletionQueue::Pluck()` to use the
  private constructor when creating the `CompletionQueue` object

  ``` C++
  class CompletionQueue :.. {
  public:
    CompletionQueue() : CompletionQueue(GRPC_CQ_NEXT, GRPC_CQ_DEFAULT_POLLINT) {}
  ..
  private:
    CompletionQueue(grpc_cq_completion_type completion_type, grpc_cq_polling_type polling_type) {
      cq_ = grpc_completion_queue_create(completion_type, polling_type, NULL);
    }
    ..
