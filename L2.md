...
``` C
/* The grpc_completion_queue_factory structure definition will remain internal and not exposed to the user */
typedef struct grpc_completion_queue_factory grpc_completion_queue_factory;

#define GRPC_CQ_CURRENT_VERSION 1
struct grpc_completion_queue_attributes {
  /* We expect to add more fields to this structure in future. So is important to pass a version number. */
     // NOTE: sree - Alternatively we can pass the struct_size as a parameter too
...
/** This is equivalent to calling the following:
   grpc_completion_queue_attributes attr = { GRPC_CQ_CURRENT_VERSION, GRPC_CQ_NEXT, GRPC_CQ_DEFAULT_POLLING };
...
/** This is equivalent to calling the following:
   grpc_completion_queue_attributes attr = {GRPC_CQ_CURRENT_VERSION, GRPC_CQ_PLUCK, GRPC_CQ_DEFAULT_POLLING };
   const grpc_completion_queue_factory* factory = grpc_completion_queue_factory_lookup(attr);
   grpc_completion_queue_create(factory, attr);
*/
...
- Change any internal code that uses `CompletionQueue::Pluck()` to use the private constructor when creating the `CompletionQueue` object
  ...
  private:
    CompletionQueue(grpc_cq_completion_type completion_type, grpc_cq_polling_type polling_type) {
      grpc_completion_queue_attributes attr = { GRPC_CQ_CURRENT_VERSION, completion_type, polling_type }
