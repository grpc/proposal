...
To support this, we will change pick_first and the petiole policies (see
[gRFC A61]) to include the `resolution_note` field in all error messages
generated by those policies.  The xds resolver will set the resolution
note in the `XdsConfig` (see [gRFC A74]) to include ambient errors for
all resources (LDS, RDS, CDS, and EDS).  If none of the resources
have ambient errors, the xds resolver will set the resolution note to
indicate the xDS node ID, which can be useful for debugging.
