...
To avoid unnecessarily committing an RPC on the client, gRPC servers *must* delay sending Response-Headers until the server's first response (a Length-Prefixed-Message) is to be sent on the stream. If the server closes the stream with an error response before sending Response-Headers, gRPC servers *must* send a Trailers-Only response to the client. This avoids the degenerate case where a server sends Response-Headers, committing the RPC on the client side and making it non-retryable, then immediately closes the stream with an error response in the Trailers.
