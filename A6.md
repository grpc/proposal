...
----
  ...
  * [Proposal](#proposal)
     ...
     * [Retry Internals](#retry-internals)
        ...
        * [Change to gRPC Wire Format](#change-to-grpc-wire-format)
...
1. The client receives a non-error response from the server. Because of the gRPC wire specification, this will always be a Response-Headers frame containing the initial metadata.
...
The [gRPC wire format](https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-WEB.md) specifies that a gRPC status can only be returned in a Trailers frame. Because the Response-Headers frame includes initial metadata, and the metadata (or its absence) is made available to the client application layer, as soon as reponse headers are received the RPC becomes committed. gRPC servers *should* delay sending response headers on a stream until the first outgoing message is sent on the stream. This avoids the degenerate case where a server sends response headers, committing the RPC on the client side and making it non-retryable, then immediately closes the stream with an error response. Instead, gRPC servers should send a Trailers-Only response when an error is generated before the first outgoing message from the server application layer.
