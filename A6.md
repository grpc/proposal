...
When a method has chosen a `hedging_policy`, the first RPC is sent immediately, as with a standard non-hedged call. After `hedging_delay_ms` has elapsed without a successful response, the second RPC will be issued. If neither RPC has received a response after `hedging_delay_ms` has elapsed again, a third RPC is sent, and so on, up to `max_requests`. In the above configuration, after 1ms there would be one outstanding hedged RPC, after 501ms there would be two outstanding hedged RPCs, and after 1001ms there would be three outstanding hedged RPCs. As with retries, gRPC call deadlines apply to the entire chain of hedged requests. The time limit starts when the original RPC is issued, and if the deadline is reached all outstanding hedged requests are aborted.
...
For each server name, the gRPC client maintains a `token_count` which is initially set to `max_tokens`. Every outgoing RPC (regardless of service or method invoked) will effect `token_count` as follows:
...
```
{
  ...
  'retry_throttling': {
    # Integer
    'max_tokens': number,
    # Floating point numbers with up to 3 decimal places are allowed.
    'token_ratio': number
  },
  ...
  'methodConfig': [
    {
      ...
      # Retry policy or hedging policy. Only one may be set.
      'retry_policy': {
        # Integer
