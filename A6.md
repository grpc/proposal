...
The reasoning behind the first scenario is that as soon as a non-error response is seen from the server, it is transmitted up to the client application. This may fundamentally change the state of the client, so we cannot safely retry if a failure occurs later in the RPC’s life. This design feature of retries necessitates a small change to the gRPC wire protocol, which is described in the [following section](#change-to-grpc-wire-format)
...
#### Change to gRPC Wire Format

If a server's response only consists of a status code, it is currently legal to send the status back in the trailers. Based on the previous section, this would mean that those RPCs would not be considered valid to retry, since the client would already have seen the initial metadata.

While implementing retries we will make the wire protocol slightly stricter, and ensure that the status is sent back in the initial metadata if that is all the server is going to send, which will give the RPC a chance of being retried if it should.
