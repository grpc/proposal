...
There are currently two implementations of the C++ callback API. One has been implemented using numerous pull requests in the [project associated with the callback API](https://github.com/grpc/grpc/projects/12). In this approach, the callback API is actually implemented on top of the C++ CQ-based async API internally, with a number of library-owned threads that call `Next` on the async CQ used in the internal implementation. Despite being built on the CQ-based async API, all the details of that API are hidden from the developer using the callback API (e.g., shutdown, polling, or even the existence of a CQ).
