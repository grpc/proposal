...
1. Operation-initiation methods: start an asynchronous activity in the RPC. These are methods provided by the class and are not virtual.
   ...
   - `StartCall` : actually causes operations to be initiated. Before this point, any `StartRead`/`StartWrite`/etc. are just backlogged. This exists to allow precise control over the initiation of streaming operations. *Client only*. (This is not needed at the server side since streaming operations at the server are released from backlog automatically by the library as soon as the application returns a reactor from the method handler.)
   ...
   - _Constraints_: There may not be more than one concurrent `StartRead` and more than one concurrent `StartWrite` in-progress for a given RPC. However, `StartRead` and `StartWrite` may be concurrent. These operations are considered done when the `OnReadDone` or `OnWriteDone` reactions is invoked. There must be exactly one call of `Finish` at the server and `StartCall` at the client. There may be no more than one use of `StartWritesDone` at the client or `StartSendInitialMetadata` at the server. Any invocation of `StartSendInitialMetadata` at the server must happen before an invocation of `StartWrite` or `Finish`.
1. Operation-completion reaction methods: notification of completion of asynchronous RPC activity. These are all virtual methods that default to an empty function (i.e., `{}`) but may be overridden by the application's reactor definition.
   ...
   - _Thread safety_: the above calls may take place concurrently, except that `OnDone` will always take place after all other reactions. No further RPC operations should be issued after `OnDone` is invoked.
